...
class CambioAlmacenScreen extends ConsumerStatefulWidget {
  const CambioAlmacenScreen({super.key});

  @override
  ConsumerState<CambioAlmacenScreen> createState() =>
      _CambioAlmacenScreenState();
}

class _CambioAlmacenScreenState extends ConsumerState<CambioAlmacenScreen>
  ...
  void initState() {
    ...
    _qrController = TextEditingController(
      text: ref.read(cambioAlmacenProvider).qrRaw,
    );
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(cambioAlmacenProvider);
    final notifier = ref.read(cambioAlmacenProvider.notifier);
    final usuario = ref.watch(authProvider).user?.usuario ?? 'OPERARIO';

    ref.listen<CambioAlmacenState>(cambioAlmacenProvider, (previous, next) {
      if (!mounted) return;
        ...
        const Expanded(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 3),
              Text(
                'Traslado de cajas con contrato legacy Screen5',
  ...
  Widget _buildStatusBanner(CambioAlmacenState state) {
  ...
  Widget _buildScanCard(
    CambioAlmacenState state,
    CambioAlmacenNotifier notifier,
  ) {
    return _buildCard(
      title: 'Escaneo de QR',
      children: [
          ...
          children: [
            ...
            Expanded(
              child: ElevatedButton.icon(
                ...
                icon:
                    state.status == CambioAlmacenStatus.parsingQr
                        ? const SizedBox(
                          width: 16,
                          height: 16,
  ...
  Widget _buildParsedCard(CambioAlmacenState state) {
      ...
      children: [
        Wrap(
          ...
          children: [
            ...
            _chip('Num. Telar', _safe(parsed.numTelar)),
            _chip('Codigo Telas', _safe(parsed.codigoTelas)),
            _chip('Orden OP', _safe(parsed.ordenOperacion)),
            _chip('Articulo', _safe(parsed.articulo)),
            _chip('Plegador', _safe(parsed.numPlegador)),
            _chip('Metro Corte', _safe(parsed.metroCorte)),
            _chip('Peso', _safe(parsed.pesoKg)),
            _chip('Fecha Corte', _safe(parsed.fechaCorte)),
            _chip('Fecha Revisado', _safe(parsed.fechaRevisado)),
      ...
      children: [
        DropdownButtonFormField<String>(
          ...
          onChanged:
              state.isBusy
                  ? null
                  : (value) => notifier.setAlmacen(value ?? 'PLANTA 1'),
          items:
              state.almacenes
                  .map(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Text(
                'Cola offline de cambio almacen',
                style: TextStyle(
                  color: CorporateTokens.navy900,
                  fontSize: 12,
                  fontWeight: FontWeight.w700,
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  icon:
                      state.status == CambioAlmacenStatus.drainingQueue
                          ? const SizedBox(
                            width: 15,
                            height: 15,
  ...
  Widget _buildQueueItem(
    CambioAlmacenQueueJobModel job,
    CambioAlmacenNotifier notifier,
    bool isBusy,
  ) {
    return Container(
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  '${job.codigoTelas} | ${job.almacen} > ${job.ubicacion}',
                  style: const TextStyle(
                    color: CorporateTokens.navy900,
                    fontSize: 11,
                    fontWeight: FontWeight.w700,
  ...
  Widget _buildActions({
    required CambioAlmacenState state,
      ...
      children: [
        DecoratedBox(
          ...
          child: ElevatedButton.icon(
            ...
            icon:
                state.status == CambioAlmacenStatus.sending
                    ? const SizedBox(
                      width: 16,
                      height: 16,
                      child: CircularProgressIndicator(
  ...
  Future<void> _scanQr(CambioAlmacenNotifier notifier) async {
    final scanned = await openQrScanner(
      context,
      title: 'Escanear QR para cambio de almacen',
    );
